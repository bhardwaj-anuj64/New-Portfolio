...
- [x] Submitted `sitemap.xml` to Google Search Console (domain property, DNS-verified) and Bing Webmaster Tools (imported from GSC) — both processed successfully, indexing requested. Also fixed a real, unrelated bug surfaced along the way: `www.anujb.dev` had no DNS record at all (never configured, not something recently broken) — added, now routes correctly alongside the apex.
- [x] Meta/OG/Twitter description shortened from 158 to 148 chars (Bing flagged 158 as too long, right at the truncation edge) — done, low-risk copy edit.
- [ ] Bing also flagged "H1 tag missing" — not actually true (`Hero.tsx` has a real `<h1>Anuj Bhardwaj</h1>`), but the raw pre-JS HTML Bing's crawler reads is empty since this is a fully client-rendered SPA with no SSR/prerendering. Google already indexes the page correctly (it executes JS), so this is a known, accepted limitation rather than a bug — revisit only if Bing ranking specifically matters enough to justify adding a prerender step.

## Security follow-ups (final pre-launch audit)

- [x] `ToolsProxyController.cs` — was forwarding any path to the internal tools-service unauthenticated; now checks against a `HashSet` allowlist of the 9 exact paths the frontend actually calls (`calibrate/rectify`, `segment/preview/magicwand`, `segment/preview/grabcut`, `segment/islands`, `segment/finalize`, `band/preview`, `band/finalize`, `mesh/from_pockets`, `mesh/from_silhouette`), 404s anything else. Closes the gap where a future tools-service route (debug endpoint, `/docs`, framework default) would've been instantly public through this proxy.
- [x] `DockerService.cs` — `GetContainersAsync` was returning every container on the host (PiHole, Home Assistant, etc., not just the portfolio's own stack). Now filters server-side via the Docker API's own `label` filter, scoped to `com.docker.compose.project=new-portfolio` (matches the `-p new-portfolio` flag `deploy.yml` already uses).
